app.controller("deductionsController",
    ...
    $scope.deductionListGrid = {
        ...
        columnDefs: [
          ...
          { name: 'deductionTitle', displayName:'Title' },
          ...
          { name: 'status.statusName', displayName:'Approval Status '},
          {
            ...
            cellTemplate:
                '<div class="ui-grid-cell-contents">'+
                    '<a href="/#!/payroll/deductions/details/?id={{row.entity.id}}">'+
                       '<button class="btn btn-primary-outline ks-no-text" title="View Details">'+
                          '<span class="la la-navicon ks-icon"></span>'+
                       '</button>'+
                    '</a>'+
                    '<a href="/#!/payroll/deductions/edit/?id={{row.entity.id}}" ng-if="row.entity.status.statusName!=\'Approved\'">'+
                    ...
                    '<a ng-if="row.entity.status.statusName==\'Approved\'">'+
                        ...
                        '</button>'+
                    '</a>'+
                    '<a ng-click="grid.appScope.deleteDeduction(row.entity.id)" ng-if="row.entity.status.statusName!=\'Approved\'">'+
    ...
    function init(){
        DeductionServices.getAllDeduction().then(function(resp){
            $scope.deductionListGrid.data = resp.data.content;
    ...
    $scope.filter={};
    $scope.employeeDeductionFilterBySearchString=function(filter){
    ...
    $scope.deleteDeduction = function(id){
       ...
       function(){
           DeductionServices.deleteDeductionById(id).then(function(resp){
               init();
           })
       });
    }
