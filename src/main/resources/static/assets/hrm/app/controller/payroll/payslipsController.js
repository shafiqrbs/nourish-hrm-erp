app.controller("payslipsController",
    ...
    $scope.salaryPaySlipsListGrid = {
        ...
        columnDefs: [
          ...
          { name: 'salaryDateCalendar.date', displayName:'Salary Date', type: 'date', cellFilter: 'date:\'dd/MM/yyyy\'' },
          ...
          { name: 'status.statusName', displayName:'Approval Status'},
          {
            ...
            cellTemplate:
                '<div class="ui-grid-cell-contents">'+
                    '<a href="/#!/payroll/salary/payslips/details/?id={{row.entity.id}}">'+
                       '<button class="btn btn-primary-outline ks-no-text" title="View Details">'+
                          '<span class="la la-navicon ks-icon"></span>'+
                       '</button>'+
                    '</a>'+
                    '<a href="javascript:void(0);" ng-click="grid.appScope.deletePayslip(row.entity.id)">'+
    ...
    function init(){
        SalaryPaySlipsServices.getAllSalaryPaySlips().then(function(resp){
            $scope.salaryPaySlipsListGrid.data = resp.data.content;
    ...
    $scope.filter={};
    $scope.employeePayslipsFilterBySearchString=function(filter){
    ...
    $scope.deletePayslip = function(id){
       swal({
           title: "System ask!",
       ...
       function(){
           SalaryPaySlipsServices.deleteSalaryPaySlipsById(id).then(function(resp){
               init();
           })
       });
    }
