app.controller("advanceSalaryController",
    ...
    $scope.advanceSalaryListGrid = {
        ...
        columnDefs: [
          ...
          { name: 'advanceSalaryTitle', displayName:'Title' },
          ...
          { name: 'status.statusName', displayName:'Approval Status '},
          {
            ...
            cellTemplate:
                '<div class="ui-grid-cell-contents">'+
                    '<a href="/#!/payroll/advancesalary/details/?id={{row.entity.id}}">'+
                       '<button class="btn btn-primary-outline ks-no-text" title="View Details">'+
                           '<span class="la la-navicon ks-icon"></span>'+
                       '</button>'+
                    '</a>'+
                    '<a href="/#!/payroll/advancesalary/edit/?id={{row.entity.id}}" ng-if="row.entity.status.statusName!=\'Approved\'">'+
                    ...
                    '<a ng-if="row.entity.status.statusName==\'Approved\'">'+
                        ...
                        '</button>'+
                    '</a>'+
                    '<a ng-click="grid.appScope.deleteAdvanceSalary(row.entity.id)" ng-if="row.entity.status.statusName!=\'Approved\'">'+
    ...
    function init(){
        AdvanceSalaryServices.getAllAdvanceSalary().then(function(resp){
            $scope.advanceSalaryListGrid.data = resp.data.content;
    ...
    $scope.filter={};
    $scope.employeeAdvanceSalaryFilterBySearchString=function(filter){
    ...
    $scope.deleteAdvanceSalary = function(id){
       swal({
           title: "System ask!",
       ...
       function(){
           AdvanceSalaryServices.deleteAdvanceSalaryById(id).then(function(resp){
               init();
           })
       });
    }
