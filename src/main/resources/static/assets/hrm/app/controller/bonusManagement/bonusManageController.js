app.controller("bonusManageController",
    function ($rootScope, $scope, $http, $location, $timeout, $filter, BaseApiUrlService, TokenService) {

        //Initialization
        $scope.baseUrl = BaseApiUrlService.getBaseUrl();
        $scope.pSearchCriteria = "";
        var urlObject = $location.absUrl().split( '/' );
        $scope.BonusTab = urlObject[7];
        ...
        $scope.openBonusTab = function (pageName) {
            $scope.BonusTab = pageName;
        }
        ...
        $scope.searchBonusProcessedEmployeeInfo = function(pSearchCriteria){
           $scope.EmployeeGridDataSource.query({
               page:1,
               pageSize:50,
               filter:{
